...
<div class="row">
    <div class="col-lg-6">
        <?= $form->field($model, 'responsible_name')->textInput() ?>
    </div>
    <div class="col-lg-6">
        <?= $form->field($model, 'semat')->textInput() ?>
    </div>
    <div class="col-lg-6">
...
<script>
    ...
    /************************teaser*************************/
//    $( "#participant-id_activity1:checkbox" ).change(function() {
//if ($this.check) {
//    var text = $(this).text();
//    $("ppp").val(text);
//}
//    });
    ...
    function changeteaserName() {
        $('.teaserNamep').css('display', 'block');
        $('.teaserName').css('display', 'none');
    }

    function chengetteaserGifName() {
        $('.teaserGifName1').css('display','block');
        $('.teaserGifName').css('display','none');
    }

    function Myteaser() {
        var id_teaser = $('#participant-teaser').val();
        $.ajax({
            type: 'POST',
            url:'<?php echo Yii::$app->getUrlManager()->createUrl('participant/findteaser')?>',
            data:{id_teaser:id_teaser},
            success:function (data) {
                $('#teaserGifX').html(data);
            }
...
//            $('#activity-id_exhibition').css('display','none');
    ...
    function Mycountry() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo Yii::$app->getUrlManager()->createUrl('participant/findcountry')?>',
