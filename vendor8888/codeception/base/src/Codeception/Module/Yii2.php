 ...
 * * When executing a request via one of the request functions the `request` and `response` components are both recreated.
...
class Yii2 extends Framework implements ActiveRecord, PartedModule
{
     ...
     * `Url::to` before doing a request, which would instantiate the request components with incorrect server params.
     */
     ...
     * ```
     * Requires `user` components to be enabled and configured.
     *
     * @param $user
    ...
    public function amLoggedInAs($user)
    {
        if (!$this->client->getApplication()->has('user')) {
            throw new ModuleException($this, 'User components is not loaded');
        }
    ...
    /**
     * To support to use the behavior of urlManager components
    ...
    /**
     * Gets a components from Yii container. Throws exception if components is not available
     *
     * ```php
     * <?php
