 ...
 * duplicate web application cache components in console config. You can use any components names.
 *
...
class CacheController extends Controller
{
    ...
    /**
     * Clears DB schema cache for a given connection components.
     *
     * ```
     * # clears cache schema specified by components id: "db"
     * yii cache/flush-schema db
     * ```
     *
     * @param string $db id connection components
    ...
    public function actionFlushSchema($db = 'db')
    {
        ...
        if ($connection === null) {
            $this->stdout("Unknown components \"$db\".\n", Console::FG_RED);
            return ExitCode::UNSPECIFIED_ERROR;
        }

        if (!$connection instanceof \yii\db\Connection) {
            $this->stdout("\"$db\" components doesn't inherit \\yii\\db\\Connection.\n", Console::FG_RED);
            return ExitCode::UNSPECIFIED_ERROR;
        ...
        try {
            ...
            $schema->refresh();
            $this->stdout("Schema cache for components \"$db\", was flushed.\n\n", Console::FG_GREEN);
        } catch (\Exception $e) {
     ...
     * @param array $caches array of cache components classes
     */
