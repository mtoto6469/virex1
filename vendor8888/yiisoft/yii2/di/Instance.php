 ...
 * ```
 *
 * And the following example shows how a class retrieves a components from a service locator:
 *
 * ```php
...
{
    /**
     * @var string the components ID, class name, interface name or alias name
     */
    public $id;
     ...
     * it will be treated as a components ID, a class/interface name or an alias, depending on the container type.
     *
     ...
     * You may specify a reference in terms of a components ID or an Instance object.
    ...
    public static function ensure($reference, $type = null, $container = null)
    {
        if (is_array($reference)) {
            ...
            throw new InvalidConfigException('The required components is not specified.');
        }
        ...
        if ($reference instanceof self) {
            try {
                ...
                throw new InvalidConfigException('Failed to instantiate components or class "' . $reference->id . '".', 0, $e);
            }
            if ($type === null || $component instanceof $type) {
                return $component;
            }

            throw new InvalidConfigException('"' . $reference->id . '" refers to a ' . get_class($component) . " components. $type is expected.");
        }
