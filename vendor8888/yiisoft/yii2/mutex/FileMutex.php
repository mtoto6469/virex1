 ...
 *
 * This components relies on PHP `flock()` function.
 *
 ...
 * ```
 *
 * > Note: this components can maintain the locks only for the single web server,
 * > it probably will not suffice in case you are using cloud server solution.
 *
 * > Warning: due to `flock()` function nature this components is unreliable when
...
class FileMutex extends Mutex
{
    ...
    /**
     * Initializes mutex components implementation dedicated for UNIX, GNU/Linux, Mac OS X, and other UNIX-like
     * operating systems.
